...
import BoardAIAnalyst from './BoardAIAnalyst';
import { arabicToUnicode, arabicMathToLatex } from '../utils/mathUtils';
import { motion } from 'motion/react';
import { GraphicsEngine, registerBuiltinCommands } from '../../clever-painter/index.js';
...
export default function SmartBoard({ isDarkMode = true, onSave, lectureTitle = 'السبورة الذكية' }: SmartBoardProps) {
  ...
  const getEventPos = (e: React.MouseEvent | React.TouchEvent): Point => {
    ...
    // تحويل من إحداثيات الشاشة → إحداثيات canvas الفعلية
    const px = (clientX - rect.left) * ratioX;
    const py = (clientY - rect.top) * ratioY;
    // zoom + pan: نعكس الـ transform
  ...
  // ── تنفيذ أمر رسم من المساعد الذكي ───────────────────────────
  const executeDrawCommand = useCallback(async (cmd: { type: string; params: Record<string, unknown> }) => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    ...
    try {
      ...
      const drawW = Math.min(canvas.width, 800);
      const drawH = Math.min(canvas.height, 600);
      ...
      // نسخ الرسم إلى السبورة الرئيسية
      ...
      console.error('[SmartBoard] Graphics error:', err);
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {/* ═══ المساعد الذكي — زر طافي لتحليل النصوص واستخراج أوامر الرسم ═══ */}
      <BoardAIAnalyst
        texts={texts}
        onExecuteDraw={executeDrawCommand}
